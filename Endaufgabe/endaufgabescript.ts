...
let order: number[] = []; //Array in der die vom Computer generierte Reihenfolge der aufleuchtendet Buttons gespeichert werden
...
function gameTurn(): void {
  ...
  if (compTurn) { // wenn der computer noch nicht fertig ist
    ...
    setTimeout( function(): void { //
      ...
      if (order[flash] == 4) bottomrightbutton();
      if (order[flash] == 5) innerbutton();
